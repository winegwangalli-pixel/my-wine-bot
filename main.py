...
query = st.text_input("💬 요청사항", placeholder="예) 방어랑 먹을 와인, 오늘 조용히 혼술, 집들이 선물 등", label_visibility="collapsed")
    ...
    if df.empty:
        ...
        with st.spinner("500여 종의 와인 리스트에서 최고의 맛돌이 찾는 중..."):
            ...
            **[절대 규칙] 반드시 제공된 [매장 재고 데이터]에 있는 '상품명'과 '가격'만 사용해. 절대 없는 와인을 지어내지 마.**
            ...
            [답변 규칙]
            1. 선정 이유를 맨 처음에 배치하여 전문성을 보여줄 것.
            2. 가격은 반드시 데이터에 있는 그대로 적을 것.
            3. "조건에 맞는 와인이 없어 다른 와인을 추천합니다"라는 말 대신, "사장님의 취향에 가장 가까운 최적의 와인을 엄선했습니다"라고 긍정적으로 말해줘.
